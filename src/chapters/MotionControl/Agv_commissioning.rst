...
In this section we will describe one kind of AGV, that is MGV a  Magnetic guided vehicle. Some of the part of MGV are used in other kind of AGVs. The difference is in the way of guidance and localization. The MGV have to follow a magnetic tap, using magnetic sensors. In this example the MGV use 4 magnetic sensors. Each magnetic sensor have 16 bits of output. The sensor is connected to a digital to canopen module in order to communicate with the controller.
...
Four motors and 4 drives are present. The drives take a velocity reference from the controller via analog signal, and send a speed feedback. The speed feedback signal is read by the controller via a high speed counter. Some digital IO are exchanged between controller and drive like drive reset, drive enable, drive ready or not in alarm, etc.
...
There are 2 laser scanner to monitor obstacles. Each laser scanner present diffierent areas, that can be activated depending on the state of the AGV. Each area have 2 or 3 ranges of action. The first one, red one, is used as a safety stop of the AGV. The senond range, middle, is used to slow down the AGV when the laser scanner detect some obstacle, and the third one is used as warning. Ther laser scanner have 4 digital inputs and 4 digital outputs.

The AGV is powered by batteries.
...
.. figure:: images/agv/dispanrde.png
    ...
    change picture put real dispan


Commissioning
=============

The first step in commissioning of any controller (plc or motion control) is to check the correct wiring of digital inputs and outputs. This step is relatively simple, using a variable monitor, online monitor or HMI we can debug the correct wiring of the signals following the electrical schematic.

Fig :ref:`iosignals` show a template panel of IO signals for debugging purposes.
...
This AGV have 2 potentiometers, every one is connected to a set of two wheels. When a set of 2 wheels connected to the same mechanical axes, rotate with different speeds, the set of wheels will rotate arround the center of the wheels coordinate system shown in fig. :ref:`agvtop2`. The angle of rotation is measured via a potentiometer connected to the origin of the coordinate system via an elastic joint.
...
The potentiometer used in this AGV have a range between 0 and 270 degrees. The AGV can steer between :math:`-30^{\circ}` and :math:`120^{\circ}`. We have to set the potentiometer in a way that the tension between pin 1 and 2 is around **3V** when the wheels angle is at :math:`0^{\circ}`.
...
.. figure:: images/agv/potentiometer.jpg
    ...
    Potentiometer connected to wheel via an elastic joint
...
After that we have to scale the analog input of the controller in order to read the correct angle. A linear relationship between the tension (or the raw value read by the analog input) and the angle in degree was supposed.

To adjust the parameters, we can use the dispan or a variable monitor. The procedure is to place the wheels at :math:`0^{\circ}` then at :math:`90^{\circ}` and read the corresponding value given by the analog input. Then assign the values to the variables ``NVRR\_POS\_0\_BIT\_SLEWING\_***`` and ``NVRR\_POS\_90\_BIT\_SLEWING\_***``. Of course that same procedure have to be done for both potentiometers the front and the rear one.

.. _figAngleAI:
.. figure:: images/agv/potentiometer.png
    :align: center
    :figwidth: 400px

    Potentiometer analog value and angle relationship

Using the dispan, we have to go to the voice F6 then choose 3 or 4 then save the parameters.
...
Every laser scanner have 4 inputs and 4 outputs. These digital signals are 4 bit code to identify the active area and the requested area.

The model of laser scanner we use, have 16 areas. The binary code is shown in fig. :ref:`laserscannerpbs`.
...
Different areas will be defined depending on the state of the agv. For example if the agv is not moving the alarm range of the 2 laser scanner should be extremely small. If the AGV is going forward the range of the front laser scanner should be bigger than the rear one and viceversa. We can define also another range for curving depending on the position of fixed obstacles.

Areas are activated using the digital inputs of the laser scanner (controller outputs). Following the codification of the laser scanner model e.g. fig.:ref:`laserscannerpbs`.
...
.. code-block:: basic
   :caption: params-rsm.stp Mechanical parameter of the AGV
   :name: lstmecparam
...
Non volatile registers are stored in memory, when RTE is loaded it retrieve the values from the memory. It is a good idea anyway to backup the values of  registers to a file, and load them in case of loss of memory.

We can save the values of registers to a file by sending the command ``uar /fb/lostreg.stp``. The file ``lostreg.stp`` is a file which contain desired registers to be saved. For example if we want to save only the values of regiters nvrr(140) and nvrr(162) we write in the file only these. When calling the command uar, only these registers are saved. There is a template file that can be used as a starting point.
